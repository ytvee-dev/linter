...
Prettier использует настройки из `prettier.js` и запускается отдельно от ESLint:
...
Нарушения форматирования показывает Prettier check, а не ESLint rule errors.
...
- **Форматирование** — Prettier исправляет отступы, кавычки, точки с запятой отдельным шагом
...
```bash
# Исправить ESLint auto-fixable ошибки и затем запустить Prettier
npm run fix
# или проверить только форматирование
...
- Отдельные проверки и запись форматирования
